...
def tokenize_function_qa(example):
    ...
    response = tokenizer(f"Answer:{example['answer']}<|end|>", add_special_tokens=False)
...
def tokenize_function_R(example):
    ...
    response = tokenizer(f"Answer:{example['answer']}.{prompt}{example['confidence']}<|end|>", add_special_tokens=False)
...
def tokenize_function_confidence(example):
    ...
    instruction = tokenizer(f"<|user|>\n\nQuestion:{example['question']}\nAnswer:{example['answer']}.{prompt}.<|end|>\n<|assistant|>\n", add_special_tokens=False)  # don't add special tokens at the beginning
    response = tokenizer(f"{example['confidence']}<|end|>", add_special_tokens=False)
